...
const PlayerCardCompact = ({
  ...
  return (
    <motion.div
      draggable={isDraggable && !injury}
      onDragStart={(e) => {
        if (isDraggable && !injury) {
      ...
      className={`group rounded-sm border transition-all duration-200 overflow-hidden ${
        injury
          ? 'bg-red-500/10 border-red-500/40 cursor-not-allowed opacity-70'
          : isStarter
            ? 'bg-cyan-500/10 border-cyan-500/40 shadow-lg shadow-cyan-500/20 cursor-move'
            : 'bg-slate-900/40 border-cyan-500/20 hover:border-cyan-400/40 cursor-move'
      }`}
    ...
    >
      <div className="flex items-center gap-2 p-2">
        {/* 프로필 사진 */}
        <div className="relative w-20 h-20 rounded-sm flex-shrink-0 pointer-events-none">
          <div className="w-20 h-20 rounded-sm overflow-hidden">
            {photoUrl && !photoError ? (
              <img
                ...
                draggable={false}
                className="w-20 h-20 object-cover object-[center_20%] rounded-sm border-2 border-white/20 group-hover:border-cyan-400/60 transition-all duration-300"
          ...
          {injury && (
            <div className="absolute -top-1 -right-1 pointer-events-auto z-10">
 ...
 */
const PremiumSquadBuilder = ({ team = "Manchester City", playerRatings = {}, onPlayerSelect = null }) => {
  const [formation, setFormation] = useState('4-3-3');
  const [squad, setSquad] = useState({ starters: {}, substitutes: [] });
  const [draggedPlayer, setDraggedPlayer] = useState(null);
  const [hoveredPos, setHoveredPos] = useState(null);
  // Removed popup states - now using onPlayerSelect callback to navigate to rating editor
  ...
  // 선수가 부상 중인지 확인
  const getPlayerInjury = useCallback((playerName) => {
  ...
  useEffect(() => {
    if (injuries.length === 0 || !rawPlayers.length || Object.keys(squad.starters).length === 0) return;

    const currentStarters = { ...squad.starters };
    let hasChanges = false;
    const removedPlayers = [];

    // 각 포지션을 체크하고 부상자가 있으면 제거
    Object.entries(currentStarters).forEach(([posKey, playerId]) => {
      const player = rawPlayers.find(p => p.id === playerId);
      if (player) {
        // 부상자인지 직접 체크
        const injury = injuries.find(inj =>
          inj.player_name && player.name &&
          inj.player_name.toLowerCase() === player.name.toLowerCase()
        );

        if (injury) {
          console.log(`🏥 부상자 스쿼드에서 제거: ${player.name} (${posKey}) - ${injury.injury_type || '부상'}`);
          removedPlayers.push(player.name);
          delete currentStarters[posKey];
          hasChanges = true;
        }
      }
    });
    ...
    if (hasChanges) {
      ...
      if (removedPlayers.length > 0) {
        console.log(`ℹ️ ${removedPlayers.length}명의 부상자가 스쿼드에서 제거되었습니다: ${removedPlayers.join(', ')}`);
      }
    }
  }, [injuries, rawPlayers]);
  ...
  const autoFill = () => {
    ...
    Object.keys(positions).forEach(posKey => {
      ...
      const available = players
        ...
        .filter(p => !getPlayerInjury(p.name)); // 부상자 제외
  ...
  return (
    <div className="text-white font-sans antialiased">
      ...
      {/* Player Detail Modal - Removed: Now navigates to rating editor via onPlayerSelect callback */}
    </div>
  );
};
