...
function PlayerRatingManager({ darkMode = false, initialTeam = null, initialPlayer = null, onRatingsUpdate = null }) {
  ...
  const [activeTab, setActiveTab] = useState('players'); // 'players', 'analytics', 'strength'
  ...
  useEffect(() => {
    if (initialTeam) {
      ...
      setActiveTab('players'); // 🔧 항상 선수 목록 탭으로 시작
  ...
  const handleTeamSelect = (team) => {
    ...
    setSelectedPlayer(null);
    setActiveTab('players'); // 팀 선택 시 항상 선수평가 탭으로 이동
  };
